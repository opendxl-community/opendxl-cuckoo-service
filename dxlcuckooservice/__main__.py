...
from dxlcuckooservice.service import CuckooService
...
if len(sys.argv) != 2:
    print "Usage: dxlcuckooservice <configuration files directory>"
    sys.exit(1)
...
# Create the cuckoo service
with CuckooService(sys.argv[1]) as cuckoo_service:
    try:
        # Run the cuckoo service
        cuckoo_service.run()
        running = True
